...
# export HEAD_COMMIT="84912ef8f5d2ab33e2e0339d9f060d879f947d58"
...
if [[ "$HELP" == 1 ]]; then
    ...
    echo "    -c|--commit: Set the head commit of the ownEyes/docker-stacks submodule (https://github.com/ownEyes/docker-stacks/commits/main). default: $HEAD_COMMIT."
    echo "    --python-only|--no-datascience-notebook: Use not the datascience-notebook from ownEyes/docker-stacks, don't install Julia and R."
...
#  && git clone https://github.com/ownEyes/docker-stacks.git $STACKS_DIR)
...
#     echo "Usage: $0 -c [sha-commit] # set the head commit of the docker-stacks submodule (https://github.com/ownEyes/docker-stacks/commits/master)."
#     echo "Exiting"
#     exit 2
#   else
...
cp -r $STACKS_DIR/images/datascience-notebook/setup-scripts .build/
cp $STACKS_DIR/images/datascience-notebook/Rprofile.site .build/
cp src/requirements-tensorflow-cuda.txt .build/
cp src/requirements-torch-cuda.txt .build/
cp src/requirements-common.txt .build/
chmod 755 .build/*
